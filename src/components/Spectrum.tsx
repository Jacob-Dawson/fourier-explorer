...
function AmplitudeSpectrum({components}: { components: FreqComponent[]}){
    ...
    return (
        <div className="flex-1 flex flex-col justify-end">
            <div className="flex items-end gap-px h-28">
                {bins.map(c => {
                    ...
                    return (
                        <div
                            key={c.freq}
                            className="flex-1 bg-amber-400 transition-all duration-300"
            ...
            <div className="mt-1 flex justify-between text-[8px] text-amber-400/25">
                <span>k=0</span>
                <span>63</span>
            </div>
            <p className="text-[8px] text-amber-400/25 mt-1 tracking-wider">
                log amplitude
            </p>
        </div>
    );
        ...
        <div className="flex-1 flex flex-col justify-end">
            <div className="flex items-end gap-px h-28">
                {bars.map((v, i) => (
                    <div
                        key={i}
                        className="flex-1 bg-amber-400 transition-all duration-500"
...
export default function Spectrum({ dftResult, sampledPoints}: SpectrumProps){
    ...
    return (
        ...
        <aside className="w-40 border-1 border-amber-400/10 p-4 flex flex-col gap-3 shrink-0">
            <span className="text-[9px] tracking-[0.25em] uppercase text-amber-400/30">
            ...
            {dftResult ? (
                ...
                <div className="flex-1 flex items-center justify-center">
                    <span className="text-[9px] text-amber-400/15 text-center leading-relaxed tracking-widest uppercase">
